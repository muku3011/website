...
<VirtualHost *:443>
	...
	<Location /authelia>
		...
		# Inject \"Back to hutta.in\" button container and light theme locking/positioning script right after the React root container
		Substitute "s~<div id=\"root\"></div>~<div id=\"root\"></div><div class=\"back-to-hutta-container\"><a href=\"https://hutta.in/\" class=\"back-to-hutta-btn\"><svg xmlns=\"http://www.w3.org/2000/svg\" fill=\"none\" viewBox=\"0 0 24 24\" stroke=\"currentColor\" stroke-width=\"2.5\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" d=\"M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18\" /></svg>Back to hutta.in</a></div><script>(function(){const b=document.body;function l(){if(b.getAttribute(\"data-theme\")!==\"light\"){b.setAttribute(\"data-theme\",\"light\")}}l();const o=new MutationObserver(l);o.observe(b,{attributes:true});function m(){const s=document.getElementById(\"sign-in-button\");const k=document.querySelector(\".back-to-hutta-container\");if(s&&k){let w=document.getElementById(\"button-group-wrapper\");if(!w){w=document.createElement(\"div\");w.id=\"button-group-wrapper\";w.className=\"button-group-wrapper\";s.parentNode.insertBefore(w,s)}if(s.parentNode!==w)w.appendChild(s);if(k.parentNode!==w)w.appendChild(k)}}const d=new MutationObserver(m);d.observe(document.documentElement,{childList:true,subtree:true});m()})();</script>~n"
	</Location>
