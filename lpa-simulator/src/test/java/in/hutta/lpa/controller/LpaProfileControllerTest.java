...
public class LpaProfileControllerTest {
    ...
    public void testProfileCrudFlow() throws Exception {
        ...
        // 2. GET all profiles
        mockMvc.perform(get("/lpa/profiles"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(2)))
                .andExpect(jsonPath("$[0].iccid", anyOf(is("89000000000000000001"), is("89000000000000000002"))));

        // 3. Enable profile 1
        mockMvc.perform(put("/lpa/profiles/89000000000000000001/enable"))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(put("/lpa/profiles/89000000000000000001/nickname")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"nickname\":\"Office Sim\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.profileNickname", is("Office Sim")));

        assertEquals("Office Sim", localProfileRepository.findById("89000000000000000001").orElseThrow().getProfileNickname());

        // 5. Disable profile 1
        mockMvc.perform(put("/lpa/profiles/89000000000000000001/disable"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.profileState", is("DISABLED")));

        assertEquals("DISABLED", localProfileRepository.findById("89000000000000000001").orElseThrow().getProfileState());

        // 6. Delete profile 2
        mockMvc.perform(delete("/lpa/profiles/89000000000000000002"))
                .andExpect(status().isOk())
