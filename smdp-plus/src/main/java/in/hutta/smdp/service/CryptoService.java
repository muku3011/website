...
public class CryptoService {
    ...
    public boolean verifyEuiccSignature(SessionContext session, String authenticateServerResponse) {
        ...
        try {
            ...
            try (ASN1InputStream asn1In = new ASN1InputStream(decoded)) {
                ...
                log.info("Parsed authenticateServerResponse as ASN.1 DER. Executing real ECDSA signature verification...");
            ...
            log.warn("Failed to parse/verify authenticateServerResponse as ASN.1 DER. Triggering dynamic fallback: using mock verification. Detail: {}", e.getMessage());
            return true;
        }
    }
    ...
    public byte[] performECDH(PublicKey euiccEphemeralPublicKey) {
        try {
            ...
            log.info("ECDH Key Agreement completed successfully. Generated shared secret of size {} bytes.", sharedSecret.length);
            return sharedSecret;
        } catch (Exception e) {
    ...
    public String generateBoundProfilePackage(String rawPayload, SessionContext session) {
        ...
        if (!"REAL_CRYPTO_ACTIVE".equals(session.getState())) {
            log.info("Session state is not REAL_CRYPTO_ACTIVE. Generating mock Bound Profile Package (BPP)...");
        ...
        log.info("Session state is REAL_CRYPTO_ACTIVE. Generating real ASN.1 DER-encoded Bound Profile Package (BPP)...");
        try {
            ...
            try {
                ...
                log.warn("Failed to encrypt profile payload via ECDH-derived key, using raw fallback: {}", e.getMessage());
            ...
            bppVector.add(new DERTaggedObject(true, 2, new DEROctetString(Base64.getDecoder().decode(getSmdpCertificate()))));
