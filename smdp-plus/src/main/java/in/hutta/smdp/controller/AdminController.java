...
public class AdminController {
    ...
    public ResponseEntity<?> importProfile(
        ...
        log.info("Admin importProfile: name={}, size={}, iccid={}", file.getOriginalFilename(), file.getSize(), iccid);
        ...
        try {
            ...
            return ResponseEntity.internalServerError().body("Error reading uploaded file: " + e.getMessage());
        }
    }
    ...
    private ResponseEntity<?> importProfileBytes(byte[] profileBytes, String overrideIccid) {
        ...
        if (iccid == null || iccid.trim().isEmpty()) {
            return ResponseEntity.badRequest()
                    .body("Error: Could not extract ICCID from the profile. Ensure it is a valid GSMA eSIM profile.");
        }
    ...
    @GetMapping("/profiles")
    public ResponseEntity<List<Profile>> getProfiles(@RequestParam(value = "state", required = false) String state) {
