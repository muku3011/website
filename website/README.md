...
| `setup_apache.sh` | Writes the `mod_auth_openidc` Apache config; reverse proxies eSIM and blog API routes; sets HTML cache control; adds `Header` directives to atomically expire `hutta_*` cookies on logout |
