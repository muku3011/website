...
public class AutheliaLdapBindInterceptor extends InMemoryOperationInterceptor {
    ...
    public AutheliaLdapBindInterceptor(AutheliaUserRepository userRepository, String adminDn, String adminPassword) {
    ...
    public void processSimpleBindResult(InMemoryInterceptedSimpleBindResult result) {
        ...
        if (bindDn.equalsIgnoreCase(adminDn)) {
            if (password.equals(adminPassword)) {
                ...
                result.setResult(new BindResult(result.getMessageID(), ResultCode.SUCCESS, null, null, new String[0], new Control[0]));
            } else {
                log.warn("Admin bind failed: invalid password");
                result.setResult(new BindResult(result.getMessageID(), ResultCode.INVALID_CREDENTIALS, null, null, new String[0], new Control[0]));
            }
            return;
        }
        ...
        try {
            boolean authenticated = userRepository.findById(username)
                    .map(user -> {
            ...
            result.setResult(new BindResult(result.getMessageID(), ResultCode.OTHER, "Internal server error during authentication", null, new String[0], new Control[0]));
        }
    }
}
